...
1. <code>type: mongoose.Schema.Types.ObjectId</code> **declares the field type** of notes as an array of **ObjectIds**.
   - MongoDB document automatically has an <code>\_id</code> field, and by **default its type** is an **ObjectId**.
     ...
     ```
2. So, when another collection (<code>User</code>) wants to “reference” this (a document created by <code>Note</code> model), it just stores the ObjectId.
...
- Configuration:
  ...
  - [output](https://webpack.js.org/concepts/#output) property tells Webpack **where to save the bundled code**. The target directory must be defined as an **absolute path**.
    ...
    - We also use [\_\_dirname](https://nodejs.org/docs/latest/api/modules.html#__dirname) which is a variable in Node that stores the **current directory path** of the project.
...
| **Data fetching** | Each endpoint returns a fixed data structure | Clients request _exactly_ what they need |
...
- **QL**
  ...
  - is a special set of rules and words that lets you ask questions (like, "_Hey, give me the name of the user with ID 1._") in a way the computer understands.
  - For example:
...
- Utility types include many handy tools ([Link](https://www.typescriptlang.org/docs/handbook/utility-types.html))
...
- But in industry, CI refers to **what happens after the actual merge happens** below.
